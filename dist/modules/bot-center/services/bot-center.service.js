...
exports.BotCenterService = void 0;
...
let BotCenterService = class BotCenterService {
    botConfigurationService;
    botMemoryService;
    databaseService;
    constructor(botConfigurationService, botMemoryService, databaseService) {
        this.botConfigurationService = botConfigurationService;
        this.botMemoryService = botMemoryService;
        this.databaseService = databaseService;
    }
    dataStore = this.createSeedData();
    async getOverview(selectedConversationId) {
    ...
    getStatus() {
        ...
        const openAiConfigured = Boolean(configuration.openai.apiKey) && !configuration.openai.apiKey.includes('*');
        return {
            connectedChannel: this.buildStatusCard('Connected Channel', configuration.evolution.isEnabled ? 'Evolution Ready' : 'Channel Disabled', configuration.evolution.isEnabled
                ? 'Evolution webhook entrypoint is enabled and ready to receive events.'
                : 'Inbound channel integration is currently disabled in configuration.', configuration.evolution.isEnabled ? 'healthy' : 'degraded'),
            aiStatus: this.buildStatusCard('AI Status', openAiConfigured ? 'Credentials Loaded' : 'Mock Fallback', openAiConfigured
                ? 'OpenAI service can generate real drafts when orchestration selects AI.'
                : 'OpenAI credentials are missing or masked, so mock drafting is active.', openAiConfigured ? 'healthy' : 'degraded'),
            backendStatus: this.buildStatusCard('Backend Status', 'NestJS Runnable', 'Main application, validation, CORS, health, webhook, and orchestration routes are registered.', 'healthy'),
            databaseStatus: this.buildStatusCard('Database Status', databaseHealth.persistenceMode === 'postgres'
                ? 'PostgreSQL Mode'
                : 'File Persistence Mode', `Persistence mode is ${databaseHealth.persistenceMode}; PostgreSQL contract remains prepared through environment-based configuration.`, databaseHealth.configured ? 'healthy' : 'degraded'),
            memoryStatus: this.buildStatusCard('Memory Status', `${memoryStats.messageRecords} records`, `Memory store currently has ${memoryStats.messageRecords} message records and ${memoryStats.summaries} summaries persisted.`, configuration.memory.enableShortTermMemory ? 'healthy' : 'degraded'),
        };
    }
    getPromptConfig() {
    ...
    async updatePromptConfig(payload) {
    ...
    getConversationRecordOrThrow(conversationId) {
        const record = this.dataStore.conversations.find((item) => item.summary.id === conversationId);
        if (!record) {
            throw new common_1.NotFoundException(`Conversation ${conversationId} was not found.`);
        }
        return record;
    }
    async buildConversationDetail(record) {
        return {
    ...
    buildMessage(id, conversationId, author, body, timestamp, state) {
        return {
            id,
            conversationId,
            author,
    ...
    buildMemoryItem(id, title, content, type, updatedAt) {
        return {
            id,
            title,
            content,
    ...
    buildStatusCard(label, value, description, state) {
        return {
            label,
            value,
            description,
...
//# sourceMappingURL=bot-center.service.js.map