...
export class AiBrainContextBuilderService {
  private static readonly baseSalesSystemPrompt = [
    ...
    '- Always answer the latest user message first, even if earlier messages discussed a different topic.',
    '- If the user changes topic, follow the new topic immediately and do not continue an older question unless the user asks for it.',
    '- Never answer using an earlier client question when a newer client message is already available.',
    '- If the user message is short, answer what can be answered now and ask at most one useful follow-up question.',
  ...
  build(params: {
    ...
    const coreSystemPrompt = [
      ...
      '',
      'LATEST USER MESSAGE PRIORITY',
      `The latest user message is: "${params.incomingMessage.trim()}".`,
      'Respond to that exact message first.',
      'If it conflicts with an earlier topic, the latest user message wins.',
      '',
      'AVAILABLE TOOLS',
      toolsDescription,
      '',
